...
// src/providers/pornobae.js
var cheerio = require("cheerio-without-node-native");
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var TMDB_BASE_URL = "https://api.themoviedb.org/3";
var BASE_URL = "https://pornobae.com";
...
function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {
  return getTMDBDetails(tmdbId, mediaType).then((mediaInfo) => {
    ...
    const searchUrl = `${mainUrl}/search/${encodeURIComponent(query)}/page/$i`;
    return fetch(searchUrl, {
      headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" }
    }).then((res) => res.text()).then((html) => {
      const $ = cheerio.load(html);
      const results = [];
      $("div.videos-list article").each((i, el) => {
    ...
    console.error("[Pornobae Scraper Error]", err.message);
    return [];
  });
}
