...
// src/providers/spankbang.js
var cheerio = require("cheerio-without-node-native");
var TMDB_API_KEY = "439c478a771f35c05022f9feabcca01c";
var TMDB_BASE_URL = "https://api.themoviedb.org/3";
var BASE_URL = "https://spankbang.com";
...
function getStreams(tmdbId, mediaType = "movie", season = null, episode = null) {
  return getTMDBDetails(tmdbId, mediaType).then((mediaInfo) => {
    ...
    return fetch(searchUrl, {
      ...
      if (!res.ok)
        throw new Error(`Spankbang search HTTP ${res.status}`);
      return res.text();
    }).then((html) => {
      ...
      return fetch(matched.url, {
        ...
        if (!res.ok)
          throw new Error(`Spankbang page HTTP ${res.status}`);
        return res.text();
      }).then((pageHtml) => {
