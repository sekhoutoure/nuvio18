...
function getStreams(tmdbId, mediaType = 'movie', season = null, episode = null) {
    return getTMDBDetails(tmdbId, mediaType)
        .then(mediaInfo => {
            ...
            .then(res => {
                if (!res.ok) throw new Error(`Eporner search HTTP ${res.status}`);
                return res.text();
            })
            .then(html => {
            ...
            .then(results => {
                ...
                .then(res => {
                    if (!res.ok) throw new Error(`Eporner page HTTP ${res.status}`);
                    return res.text();
                })
                .then(pageHtml => {
...
// Keep standard functions for stub compatibility but rely on getStreams
