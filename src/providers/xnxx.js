...
const MAIN_URL = 'https://www.xnxx.com';
...
function getStreams(tmdbId, mediaType = 'movie', season = null, episode = null) {
    return getTMDBDetails(tmdbId, mediaType)
        .then(mediaInfo => {
            ...
            .then(res => {
                if (!res.ok) throw new Error(`XNXX search HTTP ${res.status}`);
                return res.text();
            })
            .then(html => {
            ...
            .then(results => {
                ...
                console.log(`[XNXX] Matched search result: ${matched.title} -> ${matched.url}`);
                ...
                .then(res => {
                    if (!res.ok) throw new Error(`XNXX page HTTP ${res.status}`);
                    return res.text();
                })
                .then(pageHtml => {
        ...
        .catch(err => {
            console.error("[XNXX Scraper Error]", err.message);
            return [];
        });
}
